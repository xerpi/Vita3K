...
// Copies up to SCE_UID_NAMELEN bytes of src into dst and null-terminates.
inline void copy_uid_name(std::span<char, SCE_UID_NAMELEN + 1> dst, std::string_view src) {
    const std::size_t n = std::min(src.size(), static_cast<std::size_t>(SCE_UID_NAMELEN));
    if (n > 0)
...
struct SyncObjectBase {
    ...
    SyncObjectBase(SceUID uid, std::string_view name, SceUInt32 attr)
        : uid(uid)
        , attr(attr) {
        copy_uid_name(this->name, name);
    }
};
...
public:
    ...
    CondvarT(SceUID uid, std::string_view name, SceUInt32 attr, PairedMutexPtr associated_mutex)
        : SyncObjectBase(uid, name, attr)
...
public:
    Timer(SceUID uid, std::string_view name, SceUInt32 attr)
        : SyncObjectBase(uid, name, attr) {}
