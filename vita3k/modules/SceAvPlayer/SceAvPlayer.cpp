...
static void run_event_callback(EmuEnvState &emuenv, const ThreadStatePtr &thread, const PlayerPtr &player_info, uint32_t event_id, uint32_t source_id, Ptr<void> event_data) {
    if (player_info->event_manager.event_callback) {
        thread->call_guest_inline(player_info->event_manager.event_callback.address(), RegisterArgs{ { player_info->event_manager.user_data, event_id, source_id, event_data.address() } });
    }
}
...
EXPORT(int32_t, sceAvPlayerAddSource, SceUID player_handle, Ptr<const char> path) {
    ...
    if (!fs::exists(file_path) && player_info->file_manager.open_file && player_info->file_manager.close_file && player_info->file_manager.read_file && player_info->file_manager.file_size) {
        ...
        thread->call_guest_inline(player_info->file_manager.open_file.address(), RegisterArgs{ { player_info->file_manager.user_data, path.address() } });
        // TODO: support file_size > 4GB (callback function returns uint64_t, but I dont know how to get high dword of uint64_t)
        const uint32_t file_size = thread->call_guest_inline(player_info->file_manager.file_size.address(), RegisterArgs{ { player_info->file_manager.user_data } });
        ...
        while (remaining) {
            ...
            thread->call_guest_inline(player_info->file_manager.read_file.address(), RegisterArgs{ { player_info->file_manager.user_data, buf, offset, 0, buf_size } });
        ...
        free(emuenv.mem, buf);
        temp_file.close();
        thread->call_guest_inline(player_info->file_manager.close_file.address(), RegisterArgs{ { player_info->file_manager.user_data } });
