...
EXPORT(int, ksceKernelRegisterProcEventHandler, char *name, SceProcEventHandler *handler, Ptr<void> a3) {
    ...
    if (handler->create)
        emuenv.kernel.get_thread(thread_id)->call_guest_inline(handler->create.address(), RegisterArgs{ { 1, 0, a3.address() } });
    if (handler->start)
        emuenv.kernel.get_thread(thread_id)->call_guest_inline(handler->start.address(), RegisterArgs{ { 1, 0, a3.address() } });
