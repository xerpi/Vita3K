...
uint32_t start_module(EmuEnvState &emuenv, const SceKernelModuleInfo &module, SceSize args, Ptr<const void> argp) {
    ...
    if (module_start) {
        ...
        const uint32_t ret = module_thread->call_guest_on_thread(module_start.address(), args, argp.cast<void>()).value_or(0);
...
uint32_t stop_module(EmuEnvState &emuenv, const SceKernelModuleInfo &module, SceSize args, Ptr<const void> argp) {
    ...
    if (module_stop) {
        ...
        const uint32_t ret = module_thread->call_guest_on_thread(module_stop.address(), args, argp.cast<void>()).value_or(0);
        module_thread->request_host_thread_exit();
